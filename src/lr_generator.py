...
def generate_periodic_lr(lr_init, decay_steps, period_steps, total_steps, warmup_steps, global_step=0):
    """
    Applies three steps decay to generate learning rate array.
    ...
    Returns:
       np.array, learning rate array.
    """
    steps = int(total_steps / period_steps)             #5
    avr_steps = int(period_steps / decay_steps)         #5
    decay_epoch_index = [total_steps*i for i in range(1, avr_steps+1)]
    lr_each_step = []
    for i in range(total_steps):
        if i < warmup_steps:
            lr = lr_init + lr_init * i / warmup_steps
        else:
            if i < decay_epoch_index[0]:
                lr = lr_init
            elif i < decay_epoch_index[1]:
        ...
        lr_each_step.append(lr)
    lr_each_step = lr_each_step * steps
    lr_each_step = np.array(lr_each_step).astype(np.float32)[global_step:]
    return lr_each_step
