...
class OmniglotTask():             #有用，生成一个个task
    ...
    def __init__(self, character_folders, num_classes, train_num, test_num):    #num_classes=5,train_num=1,test_num=19
        self.character_folders = character_folders
        self.num_classes = num_classes
        self.train_num = train_num
        self.test_num = test_num

        class_folders = random.sample(self.character_folders, self.num_classes)     #lsit,len=5  这里实现挑选5个类

        labels = np.array(range(len(class_folders)))                                #list,len=5
        labels = dict(zip(class_folders, labels))                                   #字典，文件名-label
        samples = dict()

        self.train_roots = []
        self.test_roots = []
        for c in class_folders:                                                     #共挑选5次
            temp = [os.path.join(c, x) for x in os.listdir(c)]
            samples[c] = random.sample(temp, len(temp))                             #sample里存放5个类的所有图片且打乱

            self.train_roots += samples[c][:train_num]                              #在5个类别中取1个放在query集下
            self.test_roots += samples[c][train_num:train_num + test_num]           #在类别中取接下来2-20个各取一个放在support下
        self.train_labels = [labels[self.get_class(x)] for x in self.train_roots]   #在train_labels里存放类别，len=5
        self.test_labels = [labels[self.get_class(x)] for x in self.test_roots]     #len = 95
...
class Omniglot(FewShotDataset):
    ...
    def __getitem__(self, idx):
        # normalize = py_vision.Normalize(mean=[0,92206],std=[0.08426])
        ...
        if self.transform is not None:
            image = self.transform(image)           #这里transform直接调用不返回，不能写image = self.transform(image)
            image = image[0]
